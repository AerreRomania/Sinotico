...
namespace Sinotico
{
    public partial class RepMachines : Form
    {
        ...
        {
            var txt = "select distinct colorname from extendview where convert(date, [Workshift End Date Time], 101) between " +
                      "convert(date, '" + GetDateFrom + "', 101) and convert(date, '" + GetDateTo + "', 101) and " +
                      "charindex(+ ',' + cast([Workshift Name] as nvarchar(20)) + ',','" + MainWnd.Get_shift_array().ToString() + "' ) > 0 " +
                      "and charindex(+ ',' + cast(Gauge as nvarchar(20)) + ',','" + MainWnd.Get_fin_array().ToString() + "' ) > 0 " +
                      "order by colorname";
            ...
            {
                while (dr.Read())
                    cboMedia.Items.Add(dr[0].ToString());
            }
        ...
        private void LoadReport()
        {
            try
            {
                ...
                for (var date = new DateTime(_yearFrom, _monthFrom, GetDateFrom.Day); date <= new DateTime(_yearTo, _monthTo, GetDateTo.Day); date = date.AddDays(+1))
                {
                ...
                for (int i = _from_machine; i <= _to_machine; i++)
                {
                    ...
                    if (firstRead)
                    {
                        ...
                        foreach (DataRow dRow in _table_data.Rows)
                        {
                            ...
                            if (machine != i.ToString()) continue;
                        ...
                        if (media == 0 || workDays == 0) continue;
        ...
        private void LoadProcedure(DateTime start, DateTime end, string color)
        {
            ...
            if (string.IsNullOrEmpty(color) || color == "<All>") color = string.Empty;
        ...
        //private void btnColorTot_Click(object sender, EventArgs e)
        //{
        //    _is_squadra_filter = false;
        //    _sorted = false;
        //    _from_machine = 1;
